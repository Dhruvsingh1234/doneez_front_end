...
const VehicleSelector: React.FC = () => {
  ...
  const years = Array.from(
    new Array(currentYear - 1959),
    (val, index) => (1960 + index).toString()
  ).reverse();
  ...
  useEffect(() => {
    if (selectedYear && selectedMake) {
      ...
      fetch(
        ...
        .then((response) => response.json())
        .then((data) => {
          const models = data.results.map((model: any) => model.model);
        ...
        <div className="relative mb-4">
          ...
          {showMakeOptions && makeOptions.length > 0 && (
            <div className="absolute z-10 border border-gray-300 mt-1 max-h-60 overflow-auto bg-white rounded-md shadow-lg w-full">
              {makeOptions
                .filter((make) =>
                  make.toLowerCase().includes(makeInput.toLowerCase())
                )
                .map((make) => (
                  <div
                    key={make}
